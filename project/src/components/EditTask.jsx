...
import { FilePenLine, BookmarkX, BookUser, Menu, CalendarDays, Activity } from 'lucide-react';

const EditTask = ({ task, index, taskList, setTaskList, columnKey }) => {
  ...
  const updateHandler = (e) => {
    ...
    const updatedColumn = [...taskList[columnKey]];
    updatedColumn[index] = updatedTask;
  ...
  return (
    <div>
    <button
  className="flex bg-blue-300 xl:px-4 sm:px-2 rounded-xl font-extralight"
  onClick={(e) => {
    e.stopPropagation(); 
    setEditModal(true);
  }}
        ...
        <div className="absolute flex justify-center items-center overflow-x-hidden overflow-y-auto inset-0 bg-black bg-opacity-80">
          <div className="border-2 border-blue-600 lg:w-[35%] lg:h-[95%] bg-white shadow-2xl text-black rounded-xl p-3">
            <div className="items-center justify-around flex text-red-700">
              <h1 className="text-3xl font-medium text-indigo-600">Edit Task</h1>
